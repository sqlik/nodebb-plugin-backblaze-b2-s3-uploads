...
exports.handleRequest = async (req, res) => {
	...
	try {
		...
		if (settings.isBunnyEnabled()) {
			const signed = bunny.signUrl(key);
			url = signed.url;
			ttl = signed.ttl;
		} else {
			...
			url = await getSignedUrl(client.get(), cmd, { expiresIn: cfg.presignTtl });
			ttl = cfg.presignTtl;
		}

		const browserCache = Math.max(60, ttl - 60);
